...
test('user can add a task', async ({page }) => {
    await page.goto('http://localhost:5500/to-do-app/');
...
test('user can delete a task', async ({page }) => {
    await page.goto('http://localhost:5500/to-do-app/');
    await page.fill('#task-input', 'Test Task Delete Button');
    await page.click('#add-task');
    await page.click('.task .delete-task');
    const tasks = await page.$$eval('.task', tasks => tasks.map( task => task.textContent));
    expect(tasks).not.toContain('Test Task Delete Button')
})

test('user can mark a task as complete', async ({page }) => {
    await page.goto('http://localhost:5500/to-do-app/');
    await page.fill('#task-input', 'Test Task Complete Button');
    await page.click('#add-task');
...
test('user can filter tasks', async ({page }) => {
    await page.goto('http://localhost:5500/to-do-app/');
    await page.fill('#task-input', 'Test Task for filter');
    await page.click('#add-task');
    await page.click('.task .task-complete');
